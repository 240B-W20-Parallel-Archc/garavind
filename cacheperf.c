...
void latency() {
	...
	FILE *file;
	file = open("performance_output.csv", "a+");

	int length = (array_size)/(sizeof(int));
	struct Node* mem = (struct Node*)malloc(length*sizeof(struct Node));
	struct Node **lst = (struct Node**)malloc(length*sizeof(struct Node));
